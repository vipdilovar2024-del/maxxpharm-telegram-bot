#!/usr/bin/env python3
"""
MAXXPHARM Bot - Full functionality with roles and menu system
"""
...
def get_order_management_keyboard(order_id):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="📦 В работу", callback_data=f"process_order_{order_id}"),
...
async def cmd_help(message: types.Message):
    ...
    if role == UserRole.SUPER_ADMIN:
        ...
        help_text = (
            ...
            "• 👥 Пользователи - Просмотр клиентов\n"
        ...
        help_text = (
            "🆘 <b>Помощь MAXXPHARM Courier</b>\n\n"
            "📋 <b>Команды:</b>\n"
            "• /start - Главное меню\n"
            "• /help - Эта справка\n\n"
            "🔧 <b>Функции:</b>\n"
            "• 📦 Мои доставки - Активные доставки\n"
            "• 🗺 Карта - Навигация\n"
            "• 📞 Поддержка - Связь с диспетчером\n\n"
            "✅ Успешные доставки!"
        )
    else:  # CLIENT
        help_text = (
...
@dp.callback_query(F.data.startswith("process_order_"))
async def callback_process_order(callback: types.CallbackQuery):
    if not is_manager(callback.from_user.id):
        await callback.answer("❌ Доступ запрещен!")
        return
    
    order_id = int(callback.data.replace("process_order_", ""))
    order = next((o for o in ORDERS if o["id"] == order_id), None)
    
    if order:
        order["status"] = "IN_PROGRESS"
        await callback.answer(f"🔧 Заказ #{order_id} в работе!")
    else:
        await callback.answer("❌ Заказ не найден!")

@dp.callback_query(F.data.startswith("deliver_order_"))
async def callback_deliver_order(callback: types.CallbackQuery):
    if not is_manager(callback.from_user.id):
        await callback.answer("❌ Доступ запрещен!")
        return
    
    order_id = int(callback.data.replace("deliver_order_", ""))
    order = next((o for o in ORDERS if o["id"] == order_id), None)
    
    if order:
        order["status"] = "COMPLETED"
        await callback.answer(f"✅ Заказ #{order_id} доставлен!")
    else:
        await callback.answer("❌ Заказ не найден!")
