#!/usr/bin/env python3
"""
ПОЛНОФУНКЦИОНАЛЬНЫЙ ТЕЛЕГРАМ БОТ - ТОЧНОЕ МЕНЮ КАК НА КАРТИНКЕ
"""
...
class TelegramBot:
    ...
    def handle_start(self, message):
        ...
        if str(user_id) == self.admin_id:
            ...
            text = (
                f"👋 Добро пожаловать, {user_info.get('full_name', 'VIP Dilovar')}!\n\n"
    ...
    def handle_reports_menu(self, callback):
        ...
        if data == 'reports':
            ...
            text += f"Администраторов: {admins}\n"
            text += f"Обычных пользователей: {regular_users}\n\n"
            
            for user in users[:10]:
                text += f"• {user['full_name']} (@{user.get('username', 'N/A')}) - {user['role']}\n"
            ...
            text += f"В ожидании: {pending_orders}\n"
            text += f"Подтвержденных: {confirmed_orders}\n"
            text += f"Общая сумма: {total_amount:.2f} руб.\n\n"
            
            for order in orders[:10]:
                text += f"• Заказ {order['order_number']} - {order['customer_name']} - {order['total_amount']:.2f} руб. ({order['status']})\n"
            ...
            text += f"Общая сумма всех заказов: {total_amount:.2f} руб.\n\n"
            text += "По статусам:\n"
            ...
            for status, amount in status_amounts.items():
                ...
                text += f"{status_emoji.get(status, '❓')} {status}: {amount:.2f} руб.\n"
            ...
            text += f"👥 Пользователей: {len(users)}\n"
            text += f"📦 Заказов: {len(orders)}\n"
            text += f"💰 Общая сумма заказов: {sum(o['total_amount'] for o in orders):.2f} руб.\n"
