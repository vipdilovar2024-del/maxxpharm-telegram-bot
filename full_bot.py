...
class TelegramBot:
    ...
    def handle_text_message(self, message):
        ...
        if str(user_id) == self.admin_id:
            if text.startswith('add_user:'):
                # Формат: add_user: <telegram_id> <username> <full_name> <phone> <email> <role>
                try:
                    parts = text.split(' ', 6)
                    telegram_id = parts[1]
                    username = parts[2]
                ...
                # Формат: edit_role: <telegram_id> <new_role>
                try:
                    parts = text.split(' ', 2)
                    telegram_id = parts[1]
                    new_role = parts[2]
                ...
                # Формат: order: <order_number> <customer_name> <customer_phone> <customer_email> <products> <total_amount>
                try:
                    parts = text.split(' ', 6)
                ...
                # Формат: status: <order_id> <new_status>
                try:
                    parts = text.split(' ', 2)
    ...
    def handle_my_orders(self, callback):
        ...
        # Ищем заказы пользователя (если была была функция поиска)
        text = "📋 **Мои заказы**\n\n"
        ...
        text += "📞 Ваш телефон: " + user.get('phone', 'Не указан')
